  ...
  try {
    ...
    if (ciMode) {
      ...
      if (criticalIssues > 0) {
        ...
        // Exit 0 for warnings (don't fail build)
      }
    }
...
OPTIONS:
  --template <n>       Review template to use (default: quality)
                       Available: quality, security, performance, typescript, combined, all
  --output <format>    Output format (terminal, markdown, json, html)
  --output-file <file> Save results to file
  --ci-mode            CI/CD mode (non-interactive, sets exit codes)
...
EXAMPLES:
  ...
  code-review --ci-mode --output json  # CI mode with JSON output
...
=======
  const ciMode = args.includes('--ci-mode');
>>>>>>> fe2ce34 (Add CI/CD integration guide, team setup examples, and GitHub Actions workflow for automated code reviews)
...
=======
    ...
    if (!args.includes('--yes') && !args.includes('-y') && !ciMode) {
>>>>>>> fe2ce34 (Add CI/CD integration guide, team setup examples, and GitHub Actions workflow for automated code reviews)
      const shouldContinue = await askConfirmation(
        `\nProceed with review of ${filesToReview.length} files?`
      );
